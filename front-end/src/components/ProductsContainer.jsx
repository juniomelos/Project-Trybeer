...
const ProductsContainer = () => {
  ...
  useEffect(() => {
    totalCart()
    if (initialRender) { //To prevent to reset cart in local storage at first render
  ...
  return (
    <>
      <div className="cardsContainer">
        ...
        {productsFetching && productsDB.map((product, i) => (
          ...
          <div className="productCard" key={product.name}>
            ...
            <img data-testid={`${i}-product-img`} src={product.url_image} alt="test" height="80px" />
            <h3 data-testid={`${i}-product-name`}>
              {product.name}
            </h3>
            <div className="quantityContainer">
              <button data-testid={`${i}-product-minus`} className="productButton"
                onClick={() => handelClick('minus', product)}>-</button>
              <span data-testid={`${i}-product-qtd`} className="quantity">{quantity}</span>
              <button data-testid={`${i}-product-plus`} className="productButton"
                onClick={() => handelClick('plus', product)}>+</button>
            </div>
          </div>
        )
        )}
      </div>
      <button disabled={total === 0} data-testid="checkout-bottom-btn" onClick={() => handleGoToCheckOut()}>Ver Carrinho</button>
      <h2 data-testid="checkout-bottom-btn-value">R$ {total.toFixed(2).toString().replace('.', ',')}</h2>
